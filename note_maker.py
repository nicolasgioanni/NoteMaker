...
def process_input(file_path: str, file_type: str, user_request: str = "") -> str:
    ...
    if file_type == "text":
        ...
        try:
            ...
            # Step 2: Analyze transcript to determine if frames are needed
            decision_prompt = (
                "Decide if clarification video frames are needed\n"
                "List space seperated timestamps if yes otherwise reply 'nothing'\n\n"
                f"User request: {user_request}\n\n"
                f"Transcript:\n{content}"
            )
            response = client.chat.completions.create(
                messages=[
                    {"role": "system", "content": "Decide whether video frames are necessary based on the users request and transcript\n"},
            ...
            print("Response from decision model:", response_text)
            
            if response_text.lower() == "nothing":
                # No frames needed
                print("No frames needed for this video.")
            else:
                # Parse timestamps
                try:
                    timestamps = []
                    for ts in response_text.split():
                        try:
                            timestamps.append(float(ts))  # Direct float parsing
                        except ValueError:
                            timestamps.append(parse_time_string(ts))  # Handle "MM:SS" format
                except ValueError:
                    raise ValueError(f"Unexpected response format for timestamps: {response_text}")

                # Step 3: Extract and analyze frames at specified timestamps
                frame_analysis = []
                for ts in timestamps:
                    frame = get_frame_at_timestamp(converted_video_path, ts)

                    # Save frame as an image in the media directory
                    frame_path = f"media/frame_{int(ts)}.jpg"
                    frame.save(frame_path)

                    # Analyze the saved image
                    frame_analysis.append(analyze_image(frame_path))
                ...
                image_analysis = " ".join(frame_analysis)
